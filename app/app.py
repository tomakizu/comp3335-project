...
def register():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        if user:
            ...
            params = (username, bcrypt.generate_password_hash(password))
...
def home():
    if 'loggedin' in session:
        ...
        for encrypted_post in encrypted_posts:
                decrypted_post = (encrypted_post[0],encrypted_post[1],
...
def post():
    if request.method == 'POST' and 'title' in request.form and 'content' in request.form:
        ...
        title = request.form['title'].encode()
        content = request.form['content'].encode()
        query = "INSERT INTO post (username, title, content) VALUES (%s, %s, %s)"
        params = (username, cipher_suite.encrypt(title), cipher_suite.encrypt(content))
        cursor.execute(query, params)
        database.commit()
        return redirect(url_for('home'))
    else:
